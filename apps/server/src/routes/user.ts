...
const UPLOAD_DIR = () => resolve(process.env.TSBB_UPLOAD_DIR ?? './uploads');
...
export function userRoutes(services: Services) {
  ...
  app.get('/uploads/:name', async (c) => {
    ...
    // The name is generated by us and is a hash plus a known extension. Anything
    // else is refused outright rather than being resolved and checked, so there
    // is no path to traverse in the first place.
